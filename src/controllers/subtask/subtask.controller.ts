...
export const getAllSubTasks=async(req:AuthenticatedReq,res:Response)=>{
    ...
    if(subtasks.length <=0)
        return res.status(400).send({success:false,message:'SubTasks Are Not Found'})
    res.status(200).send({success:true,message:'SubTasks fetched Succesfully',subtasks})
}
...
export const getSubTaskById=async(req:AuthenticatedReq,res:Response)=>{
    ...
    if(!subTask)
        return res.status(400).send({success:false,message_en:'subTask Not Found'})
    return res.status(200).send({success:"subtask fetched Successfully",subTask})
}
...
export const addSubTask=async(req:AuthenticatedReq,res:Response)=>{
    ...
    await subtask.save();
    res.status(200).send({success:true,message_en:"subTask Added Successfully",subtask})
}
...
export const deleteSubTask=async(req:AuthenticatedReq,res:Response)=>{
    ...
    if(!subTask)
        return res.status(400).send({success:false,message_en:'SubTask Not Found'})
    res.status(200).send({success:true,message_en:'subTask Deleted Successfully'})
}
