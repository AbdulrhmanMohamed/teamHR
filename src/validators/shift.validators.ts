...
export const validateShift = (shift: IShift) => {
    const schema = joi.object<IShift>({
        ...
        start_day: joi.number().min(0).max(6).valid(...days).required()
            .alter({
                post: (schema) => schema.required(),
                put: (schema) => schema.forbidden(),
            }),
        end_day: joi.number().min(0).max(6).valid(...days).required()
            .custom((value: Number, helper: any) => {
                if (value <= shift.start_day) return helper.message("The end day cannot be smaller than the start day");
            })
            .alter({
                post: (schema) => schema.required(),
                put: (schema) => schema.forbidden(),
        ...
        time: joi.object({
            ...
            end_hour: joi.number().min(0).max(23).required().custom((value: any, helper: any) => {
                if (value <= shift.time.start_hour) return helper.message("The end hour cannot be smaller than the start hour");
            })
        }).alter({
            post: (schema) => schema.required(),
            put: (schema) => schema.forbidden(),
