...
export const checkUpdatePrivilage = async (req: AuthenticatedReq, res:Response, next:NextFunction) => {
    ...
    if(req.user?.role === Roles.ROOT){
        const company = await Company.findOne({owner: req.user._id, _id: user.company});
        if(company === null) return res.status(400).send({
            success: false,
            message: "employees can only be accessed in root's companies"
        });
    };
    if(req.body.company === null) return next();
...
export const checkCreationPrivilage = async (req: AuthenticatedReq, res:Response, next:NextFunction) => {
    ...
    // We need to check if root can add employees only to his companies
    if(req.user?.role === Roles.ROOT){
        const company = await Company.findOne({owner: req.user._id, _id: req.body.company});
        if(company === null) return res.status(400).send({
            success: false,
            message: "employees can only be accessed in root's companies"
        });
