...
SubscriptionSchema.pre('save', async function (next) {
    const { price_SR, price_USD, duration, sale } = (await this.populate<{ package: PackageI }>('package', 'duration price_SR price_USD sale')).package;
    const startDate = new Date(this.startDate);
    this.endDate = new Date(startDate.setMonth(startDate.getMonth() + duration))
